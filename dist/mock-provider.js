function seat(id, row, col, deck, status) {
    return { id, row, column: col, deck, status, metadata: {} };
}
function makeSeats(count, deck, cols, unavailable) {
    const seats = [];
    let seatIdx = 0;
...
export class MockProvider {
    ...
    async searchTrips(query) {
        ...
        for (const { trip, seats } of this.trips.values()) {
            if (query.origin && trip.origin !== query.origin)
                continue;
            if (query.destination && trip.destination !== query.destination)
                continue;
            ...
            if (query.vehicleType && trip.vehicleType !== query.vehicleType)
                continue;
